...
class match_class:
    ...
    def __init__(self, player1, player2,
                 ranked, Widget):  # constructor . Needs to be modified so that it takes two paramters, Player1 name and Player2 name which will be entered prior to the start of the game
        self.p1 = player1
        self.p2 = player2
        ...
        self.gui = Widget
        self.match_gui = gui_match.unranked_match(self.gui)
        self.setup_gui()
    ...
    #####################
    def setup_gui(self):
        self.match_gui.player1name.setText(self.p1.name)
        self.match_gui.player2name.setText(self.p2.name)

        self.match_gui.quit_button.clicked.connect(self.hide_gui)

        self.match_gui.base_widget.hide()
        
    def show_gui(self):
        self.match_gui.base_widget.show()
    def hide_gui(self):
        self.match_gui.base_widget.hide()
        self.gui.show()
        print("I tried")
    ...
    def roundstart(self):
        ...
        try:
            self.print_round_countdown(self.roundsPlayed+1)
    ...
    def start(self):
        ...
        try:
            ...
            if self.p1.roundsWon == 3:
                ...
                self.match_gui.text_area.append("MATCH OVER")
                self.match_gui.text_area.append("%s WINS"% self.p1.name)
                self.match_gui.base_widget.repaint()
                
                # print("Average reaction time :", self.p1.getAverage())
                # print("Best reaction time : ", self.p1.best_time)
                # self.p1.printAverages()

                end_game = end_game_object(self.p1, self.p2, self.p1)
                if (self.ranked == 1):
                ...
                self.match_gui.text_area.append("MATCH OVER")
                self.match_gui.text_area.append("%s WINS"% self.p2.name)
                self.match_gui.base_widget.repaint()
                ...
                end_game = end_game_object(self.p1, self.p2, self.p2)
                if (self.ranked == 1):
    ...
    def p1win(self):
        ...
        # set the proper label in the GUI
        self.match_gui.p1rounds[self.roundsPlayed-1].show()
        self.match_gui.base_widget.repaint()
        # write text in GUI
        self.match_gui.text_area.append("%s wins the round!" % self.p1.name)
        self.match_gui.base_widget.repaint()
        self.match_gui.text_area.append("--------- ROUND %d END ---------"% self.roundsPlayed)
        self.match_gui.base_widget.repaint()
        self.match_gui.text_area.append("")
        self.match_gui.text_area.append("")
        self.match_gui.text_area.append("")
        self.match_gui.base_widget.repaint()
    ...
    def p2win(self):
        ...
        #set the proper label in the GUI
        self.match_gui.p2rounds[self.roundsPlayed-1].show()
        self.match_gui.base_widget.repaint()

        # write text in GUI
        self.match_gui.text_area.append("%s wins the round!" % self.p2.name)
        self.match_gui.base_widget.repaint()
        self.match_gui.text_area.append("--------- ROUND %d END ---------" % self.roundsPlayed)
        self.match_gui.base_widget.repaint()
        self.match_gui.text_area.append("")
    ...
    # GUI FUNCTIONS
    def print_start_countdown(self):
        self.match_gui.text_area.append("Match starting in...")
        self.match_gui.base_widget.repaint()
        time.sleep(1)
        self.match_gui.text_area.append("3")
        self.match_gui.base_widget.repaint()
        time.sleep(1)
        self.match_gui.text_area.append("2")
        self.match_gui.base_widget.repaint()
        time.sleep(1)
        self.match_gui.text_area.append("1")
        self.match_gui.base_widget.repaint()
        time.sleep(1)
        self.match_gui.text_area.append("READY!")
        self.match_gui.base_widget.repaint()
        time.sleep(0.5)
        self.match_gui.text_area.clear()
        self.match_gui.base_widget.repaint()
        
    def print_round_countdown(self, current_round):
        self.match_gui.text_area.append("--------- ROUND %d ---------" % current_round)
        
        self.match_gui.text_area.append("Round %d starts in 3..." % current_round)
        self.match_gui.base_widget.repaint()
        time.sleep(1)
        self.match_gui.text_area.append("2")
        self.match_gui.base_widget.repaint()
...
class end_game_object():
    ...
    def write_ranked_match(self):
        ...
        db.insert_match(self.winner, self.winner, self.player2, self.winner.roundsWon, self.player2.roundsWon)     
